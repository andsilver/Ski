...
describe InterhomePriceImporter do
  let(:sales_office) { '1234' }
  let(:days) { 7 }

  describe '#initialize' do
    it 'sets @sales_office and @days from its parameters' do
      ipi = InterhomePriceImporter.new(sales_office, days)
  ...
  describe '#ftp_get' do
    it 'gets the file corresponding to the sales office and 7 days via FTP' do
      InterhomeFTP.should_receive(:get).with('price_1234_eur.xml')
      ipi = InterhomePriceImporter.new(sales_office, 7)
      ipi.ftp_get
    end

    it 'gets the file corresponding to the sales office and 14 days via FTP' do
      InterhomeFTP.should_receive(:get).with('price_1234_eur_14.xml')
      ipi = InterhomePriceImporter.new(sales_office, 14)
      ipi.ftp_get
    end
  end
  ...
  describe '#import' do
    it 'deletes all Interhome prices for @days days' do
      ...
      InterhomePriceImporter.new(sales_office, days).import([])
    end
